module load_store(
    ...
    always_comb begin
        ...
        case (mem_size)
            2'b00: wdata_masked = {24'b0, wdata[7:0]};   // Byte (explicitly padded)
            2'b01: wdata_masked = {16'b0, wdata[15:0]};  // Halfword (explicitly padded)
    ...
    always_comb begin
        ...
        if (mem_read) begin
            ...
            // Mask to the requested size with explicit padding for Verilator
            case (mem_size)
                2'b00: extracted = {24'b0, extracted[7:0]};
                2'b01: extracted = {16'b0, extracted[15:0]};
